...
function RecommendedManga() {
  ...
  return (
    <section className="my-8 p-6 bg-gray-100 rounded-lg">
      <h2 className="text-2xl font-semibold mb-6 text-gray-800">おすすめの漫画</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {mangaList.length > 0 ? (
          mangaList.map((manga) => (
            ...
            >
              <img className="w-full h-64 object-cover" src={manga.image} alt={manga.title} />
              <div className="p-4 text-left">
                <h3 className="text-lg font-medium text-gray-800 mb-1">{manga.title}</h3>
                <p className="text-sm text-gray-500">{manga.author}</p>
                <p className="text-sm text-gray-600 mt-2">{manga.description}</p>
              </div>
            </article>
          ))
        ) : (
